...
public final class View implements GLEventListener {
	...
	private void drawMode(GLAutoDrawable drawable) {
		...
		String svc = ("View center in scene: [" + FORMAT.format(p[0]) + " , " + FORMAT.format(p[1]) + "]");
		String sso = ("Scene origin in view: [" + FORMAT.format(q[0]) + " , " + FORMAT.format(q[1]) + "]");
	...
	private void drawCursor(GL2 gl) {
		...
		for (int i = 0; i < 32; i++) {
			...
			gl.glVertex2d(cursor.x + 0.05 * Math.cos(theta), cursor.y + 0.05 * Math.sin(theta));
		}
